...
export default function ImportDialog() {
  ...
  const { isImportDialogOpen, importingEvents } = useSettingsStore();
  ...
  const handleMerge = () => {
    actionAddEvents(importingEvents ?? []);
    handleClose();
  };
  const handleReplace = () => {
  ...
  const renderChooseFileBody = () => {
    return (
      <DialogContent>
        <Typography>
          Select a file to import. The file should be a JSON file containing an array of telemetry events.
        </Typography>
        <Typography>You can also drop a file onto the application to import it.</Typography>
      </DialogContent>
    );
  };
      ...
      <>
        <DialogContent sx={{ backgroundColor: 'bg.main' }}>
          <VStack sx={{ fontSize: '0.75em' }}>
            <TelemetryList events={importingEvents ?? []} allowDividers={false} allowSelection={false} />
          </VStack>
        </DialogContent>
        <DialogContent>
          <Typography>You are importing {importingEvents?.length ?? 0} items.</Typography>
          <Typography>Please choose to either merge these items with existing items, or replace everything.</Typography>
        </DialogContent>
      </>
    );
  };
  ...
  return (
    <Dialog onClose={handleClose} open={isImportDialogOpen} maxWidth={'md'} fullScreen={fullScreen}>
      <DialogTitle sx={{ fontVariant: 'small-caps', fontSize: '1.7em' }}>Import Data</DialogTitle>
