...
export default function getDownloadStringCsv(
  ...
  const getUniqueKeys = (data: TWEvent[], includeTwHeaders: boolean): string[] => {
  ...
  const flattenData = (
    ...
    return data.map((entry) => {
      ...
      const flattenedRawEvent = flattenObject(
        rawEvent,
        includeRawEvent ? 'rawEvent' : '',
        {},
        true,
      );
      const flattenedEntry = includeRawEvent
        ? ({
            ...topLevel,
            ...flattenedRawEvent,
